...
OUT_FILE = os.path.join(desktop, 'mosaic.jpeg')
...
class TargetImage:
    ...
    def get_data(self):
        ...
        small_img = large_img.resize((int(floor(w/TILE_BLOCK_SIZE)), int(floor(h/TILE_BLOCK_SIZE))), Image.ANTIALIAS)
...
def compose(original_img, tiles):
    ...
    try:
        ...
        for x in range(int(floor(mosaic.x_tile_count))):
            for y in range(int(floor(mosaic.y_tile_count))):
...
if __name__ == '__main__':
    img_path = raw_input("Path to image you want mosaic: ")
    tiles_path = raw_input("Path to images folder: ")

    if os.path.isfile(img_path) and os.path.isdir(tiles_path):
        mosaic(img_path, tiles_path)
    else:
